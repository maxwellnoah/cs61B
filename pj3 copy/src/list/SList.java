...
public class SList extends List {
  ...
  /**
   *  importList() takes an SList and copies all of its nodes
   *  into this SList
   *
   *  @param an SList to be imported
   *
   *  Performance:  runs in O(n) time, where n is the length of the list.
   *  For the purposes of the project, this should be ~O(1) time
   */
  public void importList(List ls) {
	  
	  try {
		for (ListNode cNode = ls.front(); cNode.isValidNode();
					cNode = cNode.next()) {
				this.insertBack(cNode.item());
			}
  ...
  /**
   *  containsItem() takes an Object and checks to see if any of its ListNodes
   *  contains the same item; returns true if this is the case.
   *
   *  @param item is the Object to be checked
   *
   *  Performance:  runs in O(n) time, where n is the length of the list.
   *  For the purposes of the project, this should be ~O(1) time
   */
  public boolean containsItem(Object item) {
	  ListNode cNode = this.front();
	  try {
		  while (cNode.isValidNode()) {
			  Object cItem = cNode.item();
