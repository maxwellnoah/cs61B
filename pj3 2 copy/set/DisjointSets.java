...
public class DisjointSets {
   ...
   *  REVISION:
   *    Error checking code will NOT allow root1 and root2 to combine if they
   *    are not their respective roots, or if they are identical.
   *    
  ...
  public void union(int root1, int root2) {
	...
	if (root1 == root2) { // Same root, stop!
		System.out.println("Error: Cannot union same root!");
		return;
	}
	
	if (array[root1]>0) { 		// Not a root,
		root1 = find(root1);	// So find its root
	}
	
	if (array[root2]>0) {		// same as above,
		root2 = find(root2);	// But for root2
	}
	
	if (array[root1]>0 || array[root2]>0) { //Not a root, even after
		System.out.println("Cannot union non-roots");
		return;
	}
	//End error checking
